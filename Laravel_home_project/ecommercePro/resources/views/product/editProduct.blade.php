    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-lg-8">
               ...
               <!-- <a href="{{url('/edit_product')}}"><button type="button" class="btn btn-success commn_btn mt-4"> Category List</button></a> --}} -->
                ...
                <div class="categoryForm mt-5">
                    <form action="{{url('/update_product',$edit_product->id)}}"
                     method="post" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group row">
                          <label for="p_title" class="col-sm-4 col-form-label">Product Title:</label>
                          <div class="col-sm-8">
                            <input type="text" class="form-control" name="p_title" id="p_title" value="{{$edit_product->title}}">
                          </div>
                        </div>
                        <div class="form-group row">
                          <label for="p_des" class="col-sm-4 col-form-label">Production Description:</label>
                          <div class="col-sm-8">
                            <input type="text" class="form-control" id="p_des" name="p_des" value="{{$edit_product->description}}">
                          </div>
                        </div>
                        <div class="form-group row">
                            <label for="p_price" class="col-sm-4 col-form-label">Product Price:</label>
                            <div class="col-sm-8">
                              <input type="number"  class="form-control" name="p_price" id="p_price" value="{{$edit_product->price}}">
                            </div>
                          </div>
                          <div class="form-group row">
                            <label for="p_quantity" class="col-sm-4 col-form-label">Product   Quantity :</label>
                            <div class="col-sm-8">
                              <input type="number"  class="form-control" name="p_quantity" id="p_quantity" value="{{$edit_product->quantity}}">
                            </div>
                          </div>
                          <div class="form-group row">
                            <label for="dis_price" class="col-sm-4 col-form-label">Discount Price :</label>
                            <div class="col-sm-8">
                              <input type="number"  class="form-control" name="dis_price" id="dis_price" value="{{$edit_product->discount_price}}">
                            </div>
                          </div>
                          <div class="form-group row">
                            ...
                            <div class="col-sm-8">
                              <select name="p_catagory" >
                                <option value="{{$edit_product->catagory}}">{{$edit_product->catagory}}</option>
                                @foreach ($cata_lists as $cata_list)
                                <option value="{{$cata_list->id}}">{{$cata_list->catagory_name}}</option>
                                @endforeach
                              </select>
                            </div>
                          </div>
